...
$(() => {
      ...
      success: (response) => {
        ...
        if (!response.error) {
          window.showMessage({
            title: 'E-mail envoyé',
            message:
              "Rendez-vous dans votre boîte de réception. Si vous pensez que vous n'avez pas reçu l'e-mail, veuillez vérifier votre dossier SPAM.",
            callBack: () => {
              window.open('/', '_self');
            },
          });
      ...
      success: (response) => {
        ...
        if (!response.error) {
          window.showMessage({
            ...
            message:
              'Votre mot de passe a été changé avec succès. Vous pouvez vous connecter en toute sécurité avec votre nouveau mot de passe.',
            callBack: () => {
              window.open(response.url, '_self');
            },
          });
  ...
  $('#signup-distributor-form').submit((event) => {
    ...
    const formData = new FormData(
      document.querySelector('#signup-distributor-form'),
    );
    $.ajax({
      ...
      success: (response) => {
        ...
        if (!response.error) {
          window.showMessage({
            title: 'Compte crée',
            message:
              "<p>Votre compte a été créé avec succès, mais est actuellement inactif.</p><p>Pour finaliser la création de votre compte et l'activer, vous devez nous fournir quelques pièces justificatives.</p>",
            callBack: () => {
              window.open(response.url, '_self');
            },
          });
      ...
      success: (response) => {
        ...
        if (!response.error) {
          window.showMessage({
            ...
            message:
              'Votre compte sera supprimé dans 48 heures. Si avant la fin de cette période vous souhaitez conserver votre compte, merci de nous contacter via nos pages de contact.',
            callBack: () => {
              window.open('/', '_self');
            },
          });
